...
function initMinigames() {
	...
	click_minigame.createHTML = function () {
		...
		var auto_string = "images/click_on.png";
		
		if (this.vars.autoclick == false) {auto_string = "images/click_off.png"}
		
		var automatic_button = $(document.createElement("img")) 
			automatic_button.attr("src", auto_string);
			automatic_button.attr("style", "cursor: pointer");
			automatic_button.attr("id", "click_farm_auto_button");
			automatic_button.attr("onmouseover", "tooltip('#'+this.id, 6, 13, 'Automatic Clicks', 'Clicks generated by the click farm will be automatically converted to autoclicks at a rate of 2 to 1. <br><span>' + ((minigames[8].vars.autoclick) ? 'On' : 'Off') + ''), function () {return 'Clicks generated by the click farm will be automatically converted to autoclicks at a rate of 2 to 1. <br><span>' + ((minigames[8].vars.autoclick) ? 'On' : 'Off') + ''}");
			automatic_button.attr("onmouseout", "hideTooltip()");
			automatic_button.attr("onclick", "toggleAutoClick()");
		...
		minigame_container.append($("<br>"));
		minigame_container.append($("<br>"));
		minigame_container.append(automatic_button);
	...
	click_minigame.update = function (dt) {
		...
		if (this.vars.autoclick && this.vars.stored_clicks > 0) {
			this.vars.stored_clicks -= 1;
			handleClick();
			handleClick();
		}
	...
	cryogenic_minigame.upgrades = [
		...
		[133, function () {return minigames[9].vars.frozen_time >= 1800;}, "Unlocks once you have more than 1800 seconds worth of bonuses frozen."],
		[134, function () {return minigames[9].vars.frozen_time >= 3600;}, "Unlocks once you have more than 3600 seconds worth of bonuses frozen."],
		[135, function () {return minigames[9].vars.frozen_time >= 7200;}, "Unlocks once you have more than 7200 seconds worth of bonuses frozen."],
		[136, function () {return minigames[9].vars.frozen_time >= 14400;}, "Unlocks once you have more than 14400 seconds worth of bonuses frozen."],
	...
	cryogenic_minigame.createDetails = function () {
        ...
        stats.attr("class", "expandable").attr("onclick", "$(this).children(':nth-child(2)').slideToggle('slow')");
			...
			stats_string += "Frozen Time: <span style='color:#3DCFFF;'>" + Math.floor(this.vars.frozen_time) + "s</span><br>";
			if (this.vars.target_buffs.length != 0) {
				stats_string += "Buff Frozen: <span style='color:#3DCFFF;'>";
				for (var i = 0; i < this.vars.target_buffs.length - 1; i++) {
					stats_string += buffs[this.vars.target_buffs[i]].name + ", ";
				}
				stats_string += buffs[this.vars.target_buffs[this.vars.target_buffs.length - 1]].name + ", ";
				stats_string += "</span><br>";
			}
	...
	cryogenic_minigame.update = function (dt) {
		this.vars.frozen_time += dt * this.vars.target_buffs.length;
		this.vars.maximum_buffs = 1 + upgrades[134].bought;
		
		if (this.vars.frozen_time >= 1800) {upgrades[133].makeAvailable()}
		if (this.vars.frozen_time >= 3600) {upgrades[134].makeAvailable()}
		if (this.vars.frozen_time >= 7200) {upgrades[135].makeAvailable()}
		if (this.vars.frozen_time >= 14400) {upgrades[136].makeAvailable()}
	}
	...
	fluctuation_minigame.createHTML = function () {	
		...
		flux_display.attr("id", "flux_display").attr("style", "font-size:25px;color:#00a2bc;").html("Flux: " + Math.floor(this.vars.flux_points) + " / " + Math.floor(this.vars.max_flux_points));		
		
		var flux_crash_display = $(document.createElement("div"));
		flux_crash_display.attr("id", "flux_crash_display").attr("style", "font-size:18px;color:#FA1111;").html("");
		if (this.vars.negative_time != 120) {
			flux_crash_display.html("-50% production in: " + Math.round(this.vars.negative_time) + "s or until another fluctuation is activated.");	
		}
		
		var variable_production = $(document.createElement("img"));
        variable_production.attr("src", "images/fluctuation_variable_production.png").attr("class", "fluctuation").attr("id", "fluctuation_0").attr("width", "64");
        variable_production.attr("onclick", "flux(0);");
        variable_production.attr("onmouseover", "tooltip('#'+this.id, 6, 18, 'Variable Production', 'Increases production by <span style=\"color:#00db0a;\">50%</span> for 90 seconds. <br><span>Cost: " + tweaker.minigames.flux_production_cost + " Flux</span><br><span style = \"font-size:10px;float:right\";>Hotkey: ' + hotkeys.activate_building_1.toUpperCase() + ' </span>')");
        variable_production.attr("onmouseout", "hideTooltip()");		
		
		var variable_clicks = $(document.createElement("img"));
        variable_clicks.attr("src", "images/fluctuation_variable_clicks.png").attr("class", "fluctuation").attr("id", "fluctuation_1").attr("width", "64");
        variable_clicks.attr("onclick", "flux(1);");
        variable_clicks.attr("onmouseover", "tooltip('#'+this.id, 6, 18, 'Variable Clicks', 'Increases value from clicking by <span style=\"color:#00db0a;\">75%</span> for 90 seconds. <br><span>Cost: " + tweaker.minigames.flux_clicks_cost + " Flux</span><br><span style = \"font-size:10px;float:right\";>Hotkey: ' + hotkeys.activate_building_2.toUpperCase() + ' </span>')");
        variable_clicks.attr("onmouseout", "hideTooltip()");		
		
		var variable_prices = $(document.createElement("img"));
        variable_prices.attr("src", "images/fluctuation_variable_prices.png").attr("class", "fluctuation").attr("id", "fluctuation_2").attr("width", "64");
        variable_prices.attr("onclick", "flux(2);");
        variable_prices.attr("onmouseover", "tooltip('#'+this.id, 6, 18, 'Variable Prices', 'Decreases the cost of all buildings by <span style=\"color:#00db0a;\">20%</span> for 30 seconds. <br><span>Cost: " + tweaker.minigames.flux_prices_cost + " Flux</span><br><span style = \"font-size:10px;float:right\";>Hotkey: ' + hotkeys.activate_building_3.toUpperCase() + ' </span>')");
        variable_prices.attr("onmouseout", "hideTooltip()");		
		
		var translation = $(document.createElement("img"));
        translation.attr("src", "images/fluctuation_translation.png").attr("class", "fluctuation").attr("id", "fluctuation_3").attr("width", "64");
        translation.attr("onclick", "flux(3);");
        translation.attr("onmouseover", "tooltip('#'+this.id, 6, 18, 'Translation', 'Permanently increases production by <span style=\"color:#00db0a;\">1%</span>. <br><span>Cost: " + tweaker.minigames.flux_translation_cost + " Flux</span><br><span style = \"font-size:10px;float:right\";>Hotkey: ' + hotkeys.activate_building_4.toUpperCase() + ' </span>')");
        translation.attr("onmouseout", "hideTooltip()");
		
		minigame_container.append(flux_display);
		minigame_container.append(flux_crash_display);
		minigame_container.append(document.createElement("br"));
		minigame_container.append(variable_production);
		minigame_container.append(variable_clicks);
		minigame_container.append(variable_prices);
		minigame_container.append(translation);
	...
	fluctuation_minigame.updateHTML = function () {
		$("#flux_display").html("Flux: " + Math.floor(this.vars.flux_points) + " / " + Math.floor(this.vars.max_flux_points));
		if (this.vars.negative_time != 120) {
			$("#flux_crash_display").html("-50% production in: " + Math.round(this.vars.negative_time) + "s or until another fluctuation is activated.");	
		} else {
			$("#flux_crash_display").html("");	
		}
	...
	fluctuation_minigame.createDetails = function () {
        ...
        description.attr("class", "expandable").attr("onclick", "$(this).children(':nth-child(2)').slideToggle('slow')");
            ...
            description.append(description_header);
            var description_content = $(document.createElement("div"));
            description_content.html("&nbsp;&nbsp;&nbsp;Fluctuation labs generate flux that can be used to activate powerful abilities. 120 seconds after an ability is activated, or when another ability is activate production is cut in half for 60 seconds."); 
            description_content.css("text-align", "left");
            description_content.toggle();
            description.append(description_content);
        ...
        stats.attr("class", "expandable").attr("onclick", "$(this).children(':nth-child(2)').slideToggle('slow')");
			...
			stats_string += "Flux: <span style='color:#00a2bc;'>" + Math.floor(this.vars.flux_points) + " / " + Math.floor(this.vars.max_flux_points) + "</span><br>";
			stats_string += "Total Fluctuations: <span style='color:#00a2bc;'>" + this.vars.total_fluxes + "</span><br>";
			stats_string += "Translation Bonus: <span style='color:#00a2bc;'>" + Math.round((this.vars.flux_multiplier - 1) * 100) + "%</span><br>";
            stats_content.html(stats_string); 
            stats_content.css("text-align", "left");
            stats_content.toggle();
            stats.append(stats_content);
	...
	fluctuation_minigame.update = function (dt) {
		this.vars.flux_points += dt / (this.vars.flux_time - upgrades[173].bought * 10)
		this.vars.max_flux_points = Math.floor(30 + buildings[13].count * 0.25 + upgrades[175].bought * 15);
		if (this.vars.negative_time != 120) {this.vars.negative_time -= dt;}
		
		if (this.vars.flux_points > this.vars.max_flux_points) {
			this.vars.flux_points = this.vars.max_flux_points;
		}
		
		if (this.vars.negative_time < 0) {
			buffs[37].activate(60 - upgrades[176].bought * 10);
			this.vars.negative_time = 120;
		}
