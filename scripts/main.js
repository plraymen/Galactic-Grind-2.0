/**
 * @fileOverview Creates global variables and contain logic for handling common events including ticks.
 */
...
/** Tick called many times per second to update core game logic. */
function fastTick() {
	var debug_rate = 1;
...
/** Tick called once per second to update core game logic. */
function slowTick() {
	updateBuildings();
    updateBuildingHelp();
	...
	if (CLOCK_TICKS != 0) {unlocks[0].update();$("#clock_container").show();}
}
/** Updates the position of text created on click.
...
/** Updates the rotation of the world icon.
 * @param {float} dt - Time since last frame.
 */
...
/** Updates the HTML display for credits and credits per second. */
...
/** Removes clocks ticks based on current game speed.
 * @param {float} dt - Time since last frame.
 */
function clockTicks(dt) {
	...
	if (CLOCK_TICKS < 0) {updateGameSpeed(1); CLOCK_TICKS = 0; addClockTicks(0);}
}
/** Updates game speed to the given game speed.
 * @param {int} speed - Value to update current speed with.
 */
...
/** Increases extra seconds that can be used by the user to increase speed.
 * @param {float} ticks - The amount of extra time in seconds to add.
 */
...
/** Updates the global information display. */
function updateGlobalDisplay() {
	if ($("#global_container").is(":visible")) {
		...
		var settings_string = "";
		var auto_10 = "", auto_30 = "", auto_endless = "", skip_tutorial = "", effects = "", menu = "", disable_tips = "", scientific_notation = ""; 
		...
		if (settings.scientific_notation) {scientific_notation = "checked = 'checked'"}
		...
		settings_string += "<br><span style='font-size:14px;'><br><span onmouseover='tooltip(this, 5, 0, \"Scientific Notation\", \"Determines if numbers should be displayed in scientific notation.\", false, false)' onmouseout='hideTooltip()' class='help_marker'>?</span> Scientific Notation <label onclick='stopProp(event)' class='container'><input id='scientific_notation_checkbox' onclick='changeScientificNotation(event)' type='checkbox' "+scientific_notation+"><span class='checkmark'></span></label>";
...
/** Switches the category of upgrades to display in the bought upgrades tab.
 * @param {event} event - The click event (propagation will be canceled).
 * @param {int} category - Id of the category to switch to.
 */
...
/** Shows the global information display to the user.
 * @param {element} to_show - The submenu to be open.
 */
function openGlobals(to_show) {
	if ($("#global_container").is(":visible")) {
		...
		$("#information_settings_content").hide();
		$("#information_upgrades_content").hide();
		$("#information_changelog_content").hide();
...
/** Opens the menu that allows the user to edit hotkeys.
 * @param {event} event - The click event (propagation will be stopped).
 */
...
/** Updates the save rate setting.
 * @param {event} event - The click event (propagation will be stopped).
 * @param {int} time - The value to replace the current save rate.
 */
 ...
 * @param {event} event - The click event (propagation will be stopped).
 * @param {int} rate - The value to replace the current refresh rate.
 */
...
/** Cancels propagation of the given event.
 * @param {event} event - The event (propagation will be stopped).
 */
...
/** Toggles the scientific notation setting. */
function changeScientificNotation() {
	settings.scientific_notation = !settings.scientific_notation;
	UPDATE_BUILDINGS = true;
	changeTier(CURRENT_TIER);
}
/** Toggles the skip tutorial setting. */
function changeSkipTutorial() {
	settings.skip_tutorial = !settings.skip_tutorial;
}
/** Toggles the disable tips setting. */
function changeDisableTips() {
	settings.disable_tips = !settings.disable_tips;
}
/** Toggles the menu close setting. */
function changeMenuClose() {
	settings.menu_close = !settings.menu_close;
}
/** Toggles the special effects setting. */
...
/** Updates the HTML for the autosave checkbox. */
...
/** Updates the HTML for the refresh rate checkbox. */
