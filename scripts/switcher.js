...
function initSwitches() {
	...
	var flux_switcher = new Switcher("#88f3f7", "#1a8ec9");
	...
	cyro_switcher.ratio = function () {
		return minigames[9].vars.target_buffs.length / minigames[9].vars.maximum_buffs;
	}
	cyro_switcher.tooltip = function (name) {
		$("#storage_bar" + name).attr("onmouseover", "tooltip(this, 3, 14, 'Cryogenic Lab', '<span style=\"color:#3DCFFF;\">'+ minigames[9].vars.target_buffs.length + '/' + minigames[9].vars.maximum_buffs + '</span> bonuses frozen.', function () {return '<span style=\"color:#3DCFFF;\">'+ minigames[9].vars.target_buffs.length + '/' + minigames[9].vars.maximum_buffs + '</span> bonuses frozen.'}, true)");
		$("#storage_bar" + name).attr("onmouseout", "hideTooltip();");
	}
	...
	flux_switcher.addBottom = function (name) {
		var bottom_container = $(document.createElement("span"));

		bottom_container.append(switchedFlux());
		
		return bottom_container;
	}
	flux_switcher.ratio = function () {
		var ratio = minigames[13].vars.flux_points / minigames[13].vars.max_flux_points;
		return ratio;
	}
	flux_switcher.tooltip = function (name) {
		$("#storage_bar" + name).attr("onmouseover", "tooltip(this, 3, 18, 'Fluctuation Lab', 'You currently have <span style=\"color:#00a2bc;\">'+ Math.floor(minigames[13].vars.flux_points) +'/' + Math.floor(minigames[13].vars.max_flux_points) + '</span> flux stored.', function () {return 'You currently have <span style=\"color:#00a2bc;\">'+ Math.floor(minigames[13].vars.flux_points) +'/' + Math.floor(minigames[13].vars.max_flux_points) + '</span> flux stored.'}, true)");
		$("#storage_bar" + name).attr("onmouseout", "hideTooltip();");
	}
...
function switchedClick() {
	...
	if (minigames[8].vars.autoclick == false) {auto_string = "images/click_off.png"}
		
	var automatic_button = $(document.createElement("img")) 
		automatic_button.attr("src", auto_string).attr("width", "28");
		automatic_button.attr("style", "position:absolute; left: 95px; bottom: -10px; cursor: pointer");
		automatic_button.attr("id", "click_farm_auto_button_main");
	...
	click_container.append(automatic_button);
...
/** Creates and returns extra HTML for the fluctuation mini-menu. 
 * @return {element} - The extra HTML.
 */
function switchedFlux() {
	var flux_container = $(document.createElement("div"));

		var variable_production = $(document.createElement("img"));
        variable_production.attr("src", "images/fluctuation_variable_production.png").attr("class", "fluctuation_main").attr("id", "fluctuation_main_0").attr("width", "28");
        ...
        variable_clicks.attr("src", "images/fluctuation_variable_clicks.png").attr("class", "fluctuation_main").attr("id", "fluctuation_main_1").attr("width", "28");
        ...
        variable_prices.attr("src", "images/fluctuation_variable_prices.png").attr("class", "fluctuation_main").attr("id", "fluctuation_main_2").attr("width", "28");
        ...
        translation.attr("src", "images/fluctuation_translation.png").attr("class", "fluctuation_main").attr("id", "fluctuation_main_3").attr("width", "28");
	...
	flux_container.append(variable_production);
	flux_container.append(variable_clicks);
	flux_container.append(variable_prices);
	flux_container.append(translation);
	
	return flux_container;
}
